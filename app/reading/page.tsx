...
interface PropertyMatrixBlock extends BlockBase {
  ...
  rows: { label: MultilingualString; text?: Record<string, InlineNode[]>; latex?: string; translations?: Record<string, { latex: string }>; }[];
}
    ...
    case "vocabulary_box":
      return (
        <div className="mb-8 bg-surface-container-lowest rounded-2xl shadow-sm border border-border overflow-hidden">
          <div className="flex items-stretch bg-primary-50 dark:bg-primary-950/20 pt-3 pb-3 px-3 border-b border-border">
            <div className="bg-surface-container-lowest border-2 border-primary-600 rounded-lg px-3 sm:px-5 py-1.5 flex items-center justify-center relative z-10 translate-x-3 shadow-sm min-w-[9.5rem] sm:min-w-[11rem]">
              <span className="text-primary-600 dark:text-primary-400 font-extrabold uppercase tracking-widest text-sm sm:text-base whitespace-nowrap"><BookOpen className="w-4 h-4 inline-block mr-1" /> Vocab</span>
            </div>
            <div className="bg-primary-600 flex-1 flex items-center justify-between pl-6 sm:pl-8 pr-4 py-2 rounded-lg shadow-sm">
    ...
    <>
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 sm:p-6 lg:p-8" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
        <div className="bg-surface-container-lowest rounded-[32px] shadow-2xl overflow-hidden relative flex flex-col w-full max-w-[1024px] max-h-[95vh]" onClick={(e) => e.stopPropagation()}>
          <button onClick={onClose} className="absolute top-6 right-6 z-30 w-10 h-10 flex items-center justify-center rounded-full bg-surface-container-high text-on-surface hover:bg-error hover:text-white transition-colors shadow-sm"><X size={20} /></button>
          <div className="flex flex-col md:flex-row flex-1 min-h-0 overflow-y-auto">
            <div className="flex-1 flex flex-col p-8 md:pr-10 border-b md:border-b-0 md:border-r border-border relative">
              <div className="mb-4"><span className="bg-primary-100 text-primary-800 px-3 py-1.5 rounded-lg text-xs font-extrabold uppercase tracking-widest inline-flex items-center gap-1.5">{currentLang} • Original</span></div>
              <h3 className="text-4xl font-extrabold text-on-surface leading-tight mb-4">{entry.word[currentLang] || "—"}</h3>
              <button onClick={() => speak(currentLang, "current")} disabled={isSpeaking !== null} className={`inline-flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-semibold w-fit mb-8 transition-all active:scale-95 ${isSpeaking === "current" ? "bg-primary-200 text-primary-800 animate-pulse" : "bg-surface-container text-on-surface hover:bg-surface-container-highest"}`}><Volume2 size={18} /> {isSpeaking === "current" ? "Speaking..." : "Listen"}</button>
              <div className="space-y-6">
                ...
                {entry.examples?.[currentLang]?.length > 0 && (
                  <div>
                    ...
                    <div className="space-y-3">{entry.examples[currentLang].map((ex, i) => (<div key={i} className="text-base text-on-surface-variant border-l-2 border-primary-200 pl-4 py-1">{ex}</div>))}</div>
                  </div>
                )}
            ...
            <div className="flex-1 flex flex-col p-8 md:pl-10 relative bg-surface-container-lowest/50">
              ...
              <button onClick={() => speak(targetLang, "target")} disabled={isSpeaking !== null} className={`inline-flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-semibold w-fit mb-8 transition-all active:scale-95 ${isSpeaking === "target" ? "bg-primary-200 text-primary-800 animate-pulse" : "bg-surface-container text-on-surface hover:bg-surface-container-highest"}`}><Volume2 size={18} /> {isSpeaking === "target" ? "Speaking..." : "Listen"}</button>
              <div className="space-y-6">
                ...
                {entry.examples?.[targetLang]?.length > 0 && (
                  <div>
                    <h4 className="text-[12px] font-bold text-primary-600 dark:text-primary-400 uppercase tracking-widest mb-2">Examples</h4>
                    <div className="space-y-3">{entry.examples[targetLang].map((ex, i) => (<div key={i} className="text-base text-on-surface-variant border-l-2 border-primary-200 pl-4 py-1">{ex}</div>))}</div>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="w-full bg-surface-container-low border-t border-border p-6 flex flex-col items-center shrink-0 max-h-[350px]">
              <div className="relative h-[250px] w-full flex items-center justify-center group cursor-pointer" onClick={() => setIsFullscreen(true)}>
                ...
                <div className="absolute inset-0 bg-black/0 group-hover:bg-black/5 rounded-xl transition-colors duration-300 flex items-center justify-center">
                  <div className="bg-black/70 text-white p-3 rounded-full opacity-0 group-hover:opacity-100 transition-all duration-300 transform scale-90 group-hover:scale-100 shadow-lg flex items-center gap-2"><Maximize2 size={18} /> <span className="text-sm font-medium pr-1">Click to expand</span></div>
                </div>
                {images.length > 1 && (
                  <>
              ...
              {images.length > 1 && (
                <div className="flex gap-2 justify-center mt-4">{images.map((_, idx) => (<button key={idx} onClick={(e) => { e.stopPropagation(); setCurrentImageIndex(idx); }} className={`h-2.5 rounded-full transition-all duration-300 ${idx === currentImageIndex ? "w-8 bg-primary-600" : "w-2.5 bg-surface-container-highest hover:bg-primary-300"}`} />))}</div>
              )}
            </div>
          )}
        </div>
    ...
    <aside className="w-64 shrink-0 bg-surface-container-lowest border-r border-border p-3 overflow-y-auto max-h-screen sticky top-16 hidden lg:flex lg:flex-col">
      ...
      <ul className="space-y-0.5 flex-1">
        {entries.map((entry) => {
          ...
          return (
            <li key={entry.id}>
              <button onClick={() => { const el = document.getElementById(entry.id); if (el) el.scrollIntoView({ behavior: "smooth", block: "start" }); }} className={`flex items-center gap-1.5 w-full text-left py-1 px-2 rounded text-xs transition border-l-2 ${isActive ? "bg-primary-100 text-on-primary-container dark:bg-primary-900/30 dark:text-primary-300 font-semibold border-primary-600" : "text-on-surface-variant hover:text-primary-600 dark:hover:text-primary-300 border-transparent"}`}>
...
export default function InteractiveBook() {
  ...
  useEffect(() => {
    fetch("/data/1.1.json").then((r) => r.json()).then(setLesson);
